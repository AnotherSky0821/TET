...
import { voxelToWorld, worldToVoxel, volumeCenterWorld } from "./Volume.ts";
import type { Volume as VolumeType } from "./Volume.ts";
...
const layoutColumns = defineModel<number>("layoutColumns", { default: 0 });
const syncImageBox = defineModel<boolean>("syncImageBox");
const syncWindow = defineModel<boolean>("syncWindow", { default: true });
const syncRoiSlice = defineModel<boolean>("syncRoiSlice", { default: true });
...
const angleDialStyle = (): Record<string, string> => {
  ...
  if (id == null) return { display: 'none' };
...
const angleDialPointerAngle = (e: MouseEvent, el: HTMLElement): number => {
  const r = el.getBoundingClientRect();
...
const angleDialMouseDown = (e: MouseEvent, axis: MprDialAxis) => {
  ...
  window.addEventListener('mouseup', angleDialMouseUpWindow, { once: true });
  e.preventDefault();
  e.stopPropagation();
};
...
const onRecoverYes = () => {
  ...
  const res = segStore.restoreFromPersistence({
    ...recoveryCandidate.value,
    sphere: recoveryCandidate.value.sphere ?? null,
  });
  if (!res.ok) {
...
watch(tileN, async () => {
  // App.vue のタイル数変更は tileN だけを更新するため、表示前に空箱を確保する。
  // 9 個以上では初期配列の長さを超え、未定義の box ができていた。
  const count = tileN.value ?? 0;
  while (imageBoxInfos.value.length < count) {
...
const resizeModeBoxId = ref(-1);
let resizeDragState: { boxId: number; startX: number; startY: number; startCol: number; startRow: number } | null = null;

const toggleResizeMode = (boxId: number) => {
  resizeModeBoxId.value = resizeModeBoxId.value === boxId ? -1 : boxId;
};

const onResizeDragStart = (boxId: number, e: PointerEvent) => {
  const info = imageBoxInfos.value[boxId] as VolumeImageBoxInfo | undefined;
  if (!info) return;
  resizeModeBoxId.value = boxId;
  resizeDragState = {
    boxId,
    startX: e.clientX,
    startY: e.clientY,
    startCol: info.colSpan ?? 1,
    startRow: info.rowSpan ?? 1,
  };
  window.addEventListener('pointermove', onResizeDragMove);
  window.addEventListener('pointerup', onResizeDragEnd, { once: true });
};

const onResizeDragMove = (e: PointerEvent) => {
  if (!resizeDragState) return;
  const columns = gridCols(tileN.value ?? 1);
  const gap = noGapMode.value ? 0 : GAP_PX;
  const cellW = Math.max(1, (imageBoxW.value ?? 1) + gap);
  const cellH = Math.max(1, (imageBoxH.value ?? 1) + TITLEBAR_H + gap);
  const colDelta = Math.round((e.clientX - resizeDragState.startX) / cellW);
  const rowDelta = Math.round((e.clientY - resizeDragState.startY) / cellH);
  const info = imageBoxInfos.value[resizeDragState.boxId] as VolumeImageBoxInfo | undefined;
  if (!info) return;
  info.colSpan = Math.max(1, Math.min(columns, resizeDragState.startCol + colDelta));
  info.rowSpan = Math.max(1, Math.min(3, resizeDragState.startRow + rowDelta));
  if (isDicomSliceImageBoxInfo(resizeDragState.boxId)) {
    resetDicomBoxView(info as DicomSliceImageBoxInfo);
  }
  const resizedBoxId = resizeDragState.boxId;
  nextTick().then(() => {
    if (imb.value?.[resizedBoxId]) imb.value[resizedBoxId].init();
    show();
  });
};
...
const getBoxCurrentClut = (i: number): number | undefined => {
  ...
  if (isDicomSliceImageBoxInfo(i)) return (imageBoxInfos.value[i] as DicomSliceImageBoxInfo).clut;
...
const getWindowValues = (i: number, layer: 'base' | 'overlay' = 'base'):
  { wc: number; ww: number; unit: string } | null => {
  const info = boxInfoAt(i) as any;
  if (!info) return null;
  const seriesIdx = layer === 'overlay' ? info.currentSeriesNumber1 : info.currentSeriesNumber;
  const series = seriesList[seriesIdx];
  const mod = modalityOfSeries(seriesIdx);
  const isPt = mod === 'PT' || mod === 'PET';
  const wc = layer === 'overlay' ? info.myWC1 : info.myWC;
  const ww = layer === 'overlay' ? info.myWW1 : info.myWW;
  let resolvedWc = wc;
  let resolvedWw = ww;
  if ((resolvedWc == null || resolvedWw == null) && layer === 'base' && isDicomSliceImageBoxInfo(i)) {
    const ds = series?.myDicom?.[info.currentSliceNumber];
    resolvedWc = Number(ds?.string('x00281050', 0) ?? '0');
    resolvedWw = Number(ds?.string('x00281051', 0) ?? '1');
  }
  if (resolvedWc == null || resolvedWw == null || !Number.isFinite(resolvedWc) || !Number.isFinite(resolvedWw)) return null;
  const mul = petDisplayMul(mod, series?.volume?.metadata?.suvFactor, series?.volume?.metadata?.suvOk);
  return { wc: resolvedWc * mul, ww: resolvedWw * mul, unit: isPt ? (segStore.petDisplayUnit === 'SUV' ? 'SUV' : 'Bq/ml') : mod };
};

const setWindowValues = (i: number, payload: { layer: 'base' | 'overlay'; wc: number; ww: number }) => {
  const info = boxInfoAt(i) as any;
  if (!info) return;
  const seriesIdx = payload.layer === 'overlay' ? info.currentSeriesNumber1 : info.currentSeriesNumber;
  const series = seriesList[seriesIdx];
  const mul = petDisplayMul(modalityOfSeries(seriesIdx), series?.volume?.metadata?.suvFactor, series?.volume?.metadata?.suvOk);
  const wc = payload.wc / mul;
  const ww = payload.ww / mul;
  if (payload.layer === 'overlay') { info.myWC1 = wc; info.myWW1 = ww; }
  else { info.myWC = wc; info.myWW = ww; }
  if (isAnyVolumeBox(i)) syncWindowFromBox(i, payload.wc, payload.ww);
  show();
};
...
const setClutOnBox = (i: number, clutId: number) => {
  if (!isAnyVolumeBox(i) && !isDicomSliceImageBoxInfo(i)) return;
  const d = imageBoxInfos.value[i] as VolumeImageBoxInfo | DicomSliceImageBoxInfo;
  if (clutId === -1) {
...
const ctBodyMaskForVolume = (vol: VolumeType | undefined | null): Uint8Array | undefined => {
...
const getVrShadingNumber = (i: number, key: 'ambient' | 'diffuse' | 'specularInt' | 'specularPower'): number | undefined => {
  const value = getVrShadingField(i, key);
...
const mprPlaneOf = (info: VolumeImageBoxInfo): 'axi' | 'cor' | 'sag' | null => {
  if (info.mprPlane) return info.mprPlane;
  const dir = determinePlaneDirection(info);
  if (dir === 'axial') return 'axi';
  if (dir === 'coronal') return 'cor';
  if (dir === 'sagittal') return 'sag';
  return null;
...
const rebuildObliqueMprBox = (boxId: number, orientation: THREE.Quaternion) => {
  if (!isAnyVolumeBox(boxId)) return;
  const info = imageBoxInfos.value[boxId] as VolumeImageBoxInfo;
  const plane = mprPlaneOf(info);
  if (!plane || isProjectionInfo(info)) return;
  const vol = seriesList[info.currentSeriesNumber]?.volume;
  if (!vol) return;
  const vectors = planeVectorsWorld(vol, plane);
  info.vecx = vectors.vecx.applyQuaternion(orientation);
  info.vecy = vectors.vecy.applyQuaternion(orientation);
  info.vecz = vectors.vecz.applyQuaternion(orientation);
};

// Rotate one shared 3D MPR basis, then regenerate every plane from it.
const rotateMprFamily = (sourceId: number, radians: number) => {
  const source = boxInfoAt(sourceId) as any;
  if (!source) return;
  const seriesIdx = source.currentSeriesNumber;
  const sourceInfo = source as VolumeImageBoxInfo;
  if (isProjectionInfo(sourceInfo)) return;
  const sourcePlane = mprPlaneOf(sourceInfo);
  if (!sourcePlane || !sourceInfo.vecz) return;
  const orientation = mprOrientationBySeries.get(seriesIdx) ?? new THREE.Quaternion();
  const delta = new THREE.Quaternion().setFromAxisAngle(sourceInfo.vecz.clone().normalize(), radians);
  orientation.premultiply(delta);
  mprOrientationBySeries.set(seriesIdx, orientation);
  for (let i = 0; i < imageBoxInfos.value.length; i++) {
    if (!isAnyVolumeBox(i)) continue;
...
const syncWindowFromBox = (sourceId: number, wc: number, ww: number) => {
  if (!syncWindow.value) return;
  const source = boxInfoAt(sourceId) as any;
  if (!source) return;
  const sourceSeries = isFusedImageBoxInfo(sourceId) && source.activeWindowLayer !== 'base'
    ? source.currentSeriesNumber1 : source.currentSeriesNumber;
  const sourceModality = modalityOfSeries(sourceSeries);
  if (!sourceModality) return;

  for (let i = 0; i < imageBoxInfos.value.length; i++) {
    if (i === sourceId || !isAnyVolumeBox(i)) continue;
    const target = imageBoxInfos.value[i] as any;
    if (isFusedImageBoxInfo(i)) {
      if (modalityOfSeries(target.currentSeriesNumber1) === sourceModality) {
        target.myWC1 = wc;
        target.myWW1 = ww;
      } else if (modalityOfSeries(target.currentSeriesNumber) === sourceModality) {
        target.myWC = wc;
        target.myWW = ww;
      }
    } else if (modalityOfSeries(target.currentSeriesNumber) === sourceModality) {
...
const dropFile = async (e: DragEvent, boxId?: number) => {
  isEnter.value = false;
  const reorderSrcStr = e.dataTransfer?.getData('application/x-metavol-box') ?? '';
  const reorderPlain = e.dataTransfer?.getData('text/plain') ?? '';
  const reorderId = reorderSrcStr || (reorderPlain.startsWith('metavol-box:')
    ? reorderPlain.slice('metavol-box:'.length)
    : '');
  if (reorderId && boxId != null) {
    const src = Number(reorderId);
    if (Number.isInteger(src) && src >= 0 && src < imageBoxInfos.value.length && src !== boxId) {
      const current = imageBoxInfos.value[src];
      imageBoxInfos.value[src] = imageBoxInfos.value[boxId];
      imageBoxInfos.value[boxId] = current;
      show();
    }
    return;
  }
  ...
  const seriesIdxStr = e.dataTransfer?.getData('application/x-metavol-series')
    || e.dataTransfer?.getData('text/plain')?.replace(/^metavol-series:/, '');
  if (seriesIdxStr) {
...
const onBoxDragStart = (e: DragEvent, boxId: number) => {
  if (!e.dataTransfer) return;
  e.dataTransfer.setData('application/x-metavol-box', String(boxId));
  e.dataTransfer.setData('text/plain', `metavol-box:${boxId}`);
  e.dataTransfer.effectAllowed = 'move';
};

const moveBox = (boxId: number, direction: 'left' | 'right' | 'up' | 'down') => {
  const count = tileN.value ?? 0;
  if (boxId < 0 || boxId >= count) return;
  const columns = gridCols(count);
  const row = Math.floor(boxId / columns);
  const column = boxId % columns;
  let target = boxId;
  if (direction === 'left' && column > 0) target = boxId - 1;
  if (direction === 'right' && column < columns - 1 && boxId + 1 < count) target = boxId + 1;
  if (direction === 'up' && row > 0) target = boxId - columns;
  if (direction === 'down' && boxId + columns < count) target = boxId + columns;
  if (target === boxId || target >= imageBoxInfos.value.length) return;

  [imageBoxInfos.value[boxId], imageBoxInfos.value[target]] =
    [imageBoxInfos.value[target], imageBoxInfos.value[boxId]];
  [boxOverlayDisabled.value[boxId], boxOverlayDisabled.value[target]] =
    [boxOverlayDisabled.value[target], boxOverlayDisabled.value[boxId]];
  [boxSyncEnabled.value[boxId], boxSyncEnabled.value[target]] =
    [boxSyncEnabled.value[target], boxSyncEnabled.value[boxId]];
  selectedImageBoxId.value = target;
  show();
};
...
const registerSnapshots = new WeakMap<VolumeType, any>();
...
const fusedMovingVolume = (boxId: number): VolumeType | null => {
...
const regSnapshotOf = (vol: VolumeType) => {
...
const pushRegUndo = (vol: VolumeType) => {
  const uid = vol.metadata?.seriesUID;
  if (!uid) return;
...
const setVolumeRegistration = (vol: VolumeType, params: readonly number[]) => {
...
const applyWindowLevelDrag = (id: number, mvX: number, mvY: number) => {
  ...
  if (isAnyVolumeBox(id)) syncWindowFromBox(id, wc, ww);
  show();
};
...
const mouseMove = (e: MouseEvent) => {
  ...
  // MPR角度調整モードは回転円からのみ操作する。画像本体のドラッグは行わない。
  if (angleAdjustMode.value) return;
    ...
    if (e.buttons == 1) {
      ...
      doOneOrAllSamePlane(id, (i:number) => {
        if (isDicomSliceImageBoxInfo(i)){
          ...
          const sliceInfo = getDicomSliceImageBoxInfo(i);
          const zoom = sliceInfo.zoom ?? 1;
          sliceInfo.zoom = zoom * r;
          showImage(i);
        }else{
...
const appendNewSeriesAsBoxes = (startSeriesCount: number) => {
  ...
  for (let k = 0; k < newPrimaryIdxs.length && oldTileN + k < targetTileN; k++) {
    ...
    const modality = (s.volume?.metadata?.modality
      ?? s.myDicom?.[0]?.string('x00080060') ?? '').toUpperCase();
    const shouldAutoReformat = modality === 'PT' || modality === 'PET'
      || modality === 'CT' || modality === 'MR';
    if (isNiftiOnly) {
      promoteBoxToVolume(boxIdx, seriesIdx);
    } else if (modality === 'PT' || modality === 'PET') {
      // PT は従来どおり SUV/CLUT の既定値を持つ Volume 化経路を使う。
      if (ensureVolume_(seriesIdx)) promoteBoxToVolume(boxIdx, seriesIdx);
    } else if (shouldAutoReformat) {
      // mpr_ は native box の DICOM WC/WW を引き継ぐ。promoteBoxToVolume は
      // modality 既定値を入れるため、原画と表示数字がずれてしまう。
      ...
      if (mpr_(seriesIdx, boxIdx)) setPlaneOnBox(boxIdx, 'axi');
    } else {
      // volume 化できない transfer syntax 等は native DICOM にフォールバック。
      const info = imageBoxInfos.value[boxIdx] as DicomSliceImageBoxInfo;
      info.currentSeriesNumber = seriesIdx;
      info.currentSliceNumber = 0;
      info.description = seriesSummaries.value[seriesIdx]?.description ?? '';
...
//   - PT/PET/CT/MR DICOM は読み込み直後から Volume の axial MPR で表示する。
//     その他の DICOM は native のまま。native image は titlebar の plane menu の
//     "Original DICOM" から戻せる。
//   - NIfTI volume-only はそのまま Volume Box にする。
...
const autoLayoutAfterLoad = () => {
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (isNiftiOnly) {
      ...
      // PT は従来どおり SUV/CLUT の既定値を持つ Volume 化経路を使う。
      if (ensureVolume_(seriesIdx)) promoteBoxToVolume(i, seriesIdx);
    } else if (shouldAutoReformat) {
      // mpr_ は native box の DICOM WC/WW を引き継ぐ。
      const info = imageBoxInfos.value[i] as DicomSliceImageBoxInfo;
      info.currentSeriesNumber = seriesIdx;
      info.currentSliceNumber = 0;
      info.description = seriesSummaries.value[seriesIdx]?.description ?? '';
      resetDicomBoxView(info);
      if (mpr_(seriesIdx, i)) setPlaneOnBox(i, 'axi');
    } else {
      // volume 化できない transfer syntax 等は native DICOM にフォールバック。
      const info = imageBoxInfos.value[i] as DicomSliceImageBoxInfo;
      info.currentSeriesNumber = seriesIdx;
...
const loadNii = async (arraybuffer: ArrayBuffer, filename?: string) => {
  ...
  if (nifti.isCompressed(arraybuffer)){
    ...
    arraybuffer = decompressed.slice().buffer;
...
const showImage = (i:number) => {
  ...
  let drawPromise: Promise<unknown> | void = undefined;
    ...
    try {
      if (dataSet === undefined) {
        ...
        if (info.zoom == null){
          ...
          const fitW = boxRenderWidth(i) / cols;
          const fitH = Math.max(1, boxRenderHeight(i)) / rows;
...
const screenToWorld = (imageBoxNumber: number, x: number, y:number) => {
  ...
  world.add(a.centerInWorld)
      .addScaledVector(a.vecx, x - boxRenderWidth(imageBoxNumber)/2)
...
const screenToWorldDicomSlice = (boxId: number, cx: number, cy: number): THREE.Vector3 | null => {
  ...
  const canvasW = boxRenderWidth(boxId);
  const canvasH = boxRenderHeight(boxId);
...
const screenToVoxelAny = (boxId: number, cx: number, cy: number): [number, number, number] | null => {
  ...
  if (isDicomSliceImageBoxInfo(boxId)) {
    ...
    const zoom = info.zoom ?? 1;
    const fx = (cx - boxRenderWidth(boxId) / 2) / zoom + cols / 2 + info.centerX;
    const fy = (cy - boxRenderHeight(boxId) / 2) / zoom + rows / 2 + info.centerY;
...
const voxelToScreenAny = (boxId: number, vx: number, vy: number, vz: number): [number, number] | null => {
  ...
  if (isDicomSliceImageBoxInfo(boxId)) {
    ...
    if (!cols || !rows) return null;
    const zoom = info.zoom ?? 1;
    const cx = (vx - cols / 2 - info.centerX) * zoom + boxRenderWidth(boxId) / 2;
    const cy = (vy - rows / 2 - info.centerY) * zoom + boxRenderHeight(boxId) / 2;
    return [cx, cy];
  }
  if (isAnyVolumeBox(boxId)) {
    const j = seriesIndexOfBox(boxId);
...
const rebuildSeriesSummaries = () => {
  ...
  for (let i = 0; i < seriesList.length; i++){
    ...
    out.push({
      ...
      transferSyntaxReason: 'reason' in tsInfo ? (tsInfo.reason ?? '') : '',
      acquisitionTime,
      studyDate,
      studyUID,
...
const volumeForExperiment = (): { idx: number; vol: VolumeType } | null => {
  const cand: number[] = [];
...
const gridCols = (n: number) => {
  if ((layoutColumns.value ?? 0) > 0) {
    return Math.max(1, Math.min(6, Math.floor(layoutColumns.value!)));
  }
  if (n <= 1) return 1;
  if (n <= 2) return 2;
  if (n <= 4) return 2;
...
const boxColSpan = (i: number): number => {
  const max = gridCols(tileN.value ?? 1);
  return Math.max(1, Math.min(max, Math.floor((imageBoxInfos.value[i] as VolumeImageBoxInfo | undefined)?.colSpan ?? 1)));
};
const boxRenderWidth = (i: number): number => {
  const span = boxColSpan(i);
  const gap = noGapMode.value ? 0 : GAP_PX;
  return (imageBoxW.value ?? 0) * span + gap * (span - 1);
};
...
const boxCellStyle = (i: number): Record<string, string> => {
  const span = boxRowSpan(i);
  const colSpan = boxColSpan(i);
  const style: Record<string, string> = {};
  if (span >= 2) style.gridRow = `span ${span}`;
  if (colSpan >= 2) style.gridColumn = `span ${colSpan}`;
  return style;
};

const resizeBox = (boxId: number, payload: { axis: 'row' | 'column'; delta: number }) => {
  const info = imageBoxInfos.value[boxId] as VolumeImageBoxInfo | undefined;
  if (!info) return;
  const maxColumns = gridCols(tileN.value ?? 1);
  if (payload.axis === 'column') {
    info.colSpan = Math.max(1, Math.min(maxColumns, (info.colSpan ?? 1) + payload.delta));
  } else {
    info.rowSpan = Math.max(1, Math.min(3, (info.rowSpan ?? 1) + payload.delta));
  }
  if (isDicomSliceImageBoxInfo(boxId)) resetDicomBoxView(info as DicomSliceImageBoxInfo);
  autoFitMode.value = true;
  applyAutoFit();
  nextTick().then(() => {
    if (imb.value?.[boxId]) imb.value[boxId].init();
    show();
  });
};
...
// Different modalities often have different FOVs (for example CT 240 mm / PT 500 mm).
// Return a common screen scale so the same anatomical region occupies a similar size.
const scalePlaneToReference = (
  source: VolumeType,
  reference: VolumeType,
  plane: 'axi' | 'cor' | 'sag',
  vectors: { vecx: THREE.Vector3; vecy: THREE.Vector3; vecz: THREE.Vector3 },
) => {
  const fov = (v: VolumeType): [number, number] => {
    if (plane === 'cor') return [v.nx * v.vectorX.length(), v.nz * v.vectorZ.length()];
    if (plane === 'sag') return [v.ny * v.vectorY.length(), v.nz * v.vectorZ.length()];
    return [v.nx * v.vectorX.length(), v.ny * v.vectorY.length()];
  };
  const src = fov(source);
  const ref = fov(reference);
  const scale = Math.min(ref[0] / Math.max(src[0], 1e-6), ref[1] / Math.max(src[1], 1e-6));
  if (Number.isFinite(scale) && scale > 0) {
    vectors.vecx.multiplyScalar(scale);
    vectors.vecy.multiplyScalar(scale);
  }
  return vectors;
};
...
// Fusion Plus: dense 2x3 clinical comparison layout.
//   [ CT axial ][ Fusion axial ][ PT axial ]
//   [ PT coronal ][ Fusion coronal ][ Fusion sagittal ]
const setupFusionPlus6up = async () => {
  const { petIdx, ctIdx } = resolvePetCtIndices();
  if (petIdx < 0 || ctIdx < 0) { alert('Both a PT and a CT series are required.'); return; }
  if (!seriesList[petIdx].volume && seriesList[petIdx].myDicom) mpr_(petIdx);
  if (!seriesList[ctIdx].volume && seriesList[ctIdx].myDicom) mpr_(ctIdx);
  ...
  const p1 = voxelToWorld_(new THREE.Vector3(ct.nx, ct.ny, ct.nz), ctIdx);
  const center = p0.add(p1).divideScalar(2);
  const makeFusion = (plane: 'axi' | 'cor' | 'sag', description: string): FusedVolumeImageBoxInfo => {
    const { vecx, vecy, vecz } = planeVectorsWorld(ct, plane);
    return {
      centerInWorld: center.clone(), vecx, vecy, vecz,
      clut: 0, clut1: 2,
      currentSeriesNumber: ctIdx, currentSeriesNumber1: petIdx,
      description, myWC: 40, myWW: 400, myWC1: 3, myWW1: 6,
    } as FusedVolumeImageBoxInfo;
  };

  imageBoxInfos.value[0] = makeVolumeBoxForPlane(ctIdx, 'axi', 'CT axial', 0, { wc: 40, ww: 400 });
  imageBoxInfos.value[1] = makeFusion('axi', 'Fusion axial');
  const ptAxial = makeVolumeBoxForPlane(petIdx, 'axi', 'PT axial', 1, { wc: 3, ww: 6 });
  const ptCoronal = makeVolumeBoxForPlane(petIdx, 'cor', 'PT coronal', 1, { wc: 3, ww: 6 });
  scalePlaneToReference(pet, ct, 'axi', ptAxial);
  scalePlaneToReference(pet, ct, 'cor', ptCoronal);
  imageBoxInfos.value[2] = ptAxial;
  imageBoxInfos.value[3] = ptCoronal;
  imageBoxInfos.value[4] = makeFusion('cor', 'Fusion coronal');
  imageBoxInfos.value[5] = makeFusion('sag', 'Fusion sagittal');

  tileN.value = 6;
  selectedImageBoxId.value = 1;
  refreshSegStoreVolumeRefs();
  autoFitMode.value = true;
  applyAutoFit();
  syncImageBox.value = true;
...
<template>
  ...
  >
    <div class="mv-inspector-header">
      <span class="mv-section-title">セグメンテーション</span>
      <v-spacer />
        <v-btn
        icon
        size="x-small"
      ...
      >
        ...
        <v-tooltip activator="parent" location="bottom">右サイドバーを非表示（セグメンテーション）</v-tooltip>
      </v-btn>
    </div>
    <SegmentationPanel
      ref="segPanelRef"
  ...
  <div class="mv-imagearea" id="hello"
    ...
    <div v-if="(tileN ?? 0) === 0" class="mv-imagearea-empty">
      ...
      <span class="mv-imagearea-empty-title">ファイルをドロップしてスタート</span>
      <span class="mv-imagearea-empty-hint">DICOM フォルダまたは NIfTI ファイル（.nii / .nii.gz）</span>
      <div class="mt-3">
        <v-btn
          color="primary"
          variant="flat"
        ...
        >
          ファイルを選択…
          <v-tooltip activator="parent" location="bottom">DICOMまたはNIfTIファイルを選択 (またはここにドロップ)</v-tooltip>
        </v-btn>
        <input
          ref="hiddenLoadInput"
          type="file"
      ...
      <span class="mv-imagearea-empty-hint mv-empty-link-hint mt-3">
        後で追加するには、左上隅の<v-icon icon="mdi-menu" size="small" />メニューを使用してください。
      </span>
      <span class="mv-imagearea-empty-hint mv-empty-link-hint">
        前のセッションを復元しますか？最初に画像ファイルをドロップし、次に
        <v-icon icon="mdi-camera-outline" size="small" />スナップショットメニュー（上部バー） → スナップショットの読み込みを使用してください。
      </span>
      <span class="mv-imagearea-empty-hint mv-empty-link-hint">
        リモートファイル: DICOM/NIfTI を <code>?url=https://your-host/scan.nii.gz</code> として指定してください
        （複数の <code>?url=</code> パラメータに対応。CORS を許可したホストのみ）
      </span>
    </div>
    <div class="mv-tile-grid" :class="{ 'is-no-gap': noGapMode }" :style="gridStyle">
      <imagebox
        ...
        @boxDragStart="(e: DragEvent) => onBoxDragStart(e, i-1)"
        @moveBox="(direction: 'left' | 'right' | 'up' | 'down') => moveBox(i-1, direction)"
        @resizeBox="(p: { axis: 'row' | 'column'; delta: number }) => resizeBox(i-1, p)"
        @toggleResizeMode="toggleResizeMode(i-1)"
        @resizeDragStart="(e: PointerEvent) => onResizeDragStart(i-1, e)"
        @setWindow="(p: { layer: 'base' | 'overlay'; wc: number; ww: number }) => setWindowValues(i-1, p)"
        :isEnter="isEnter"
        ...
        :window-values="getWindowValues(i-1)"
        :window-values2="getWindowValues(i-1, 'overlay')"
        ...
        :vr-shading-ambient="getVrShadingNumber(i-1, 'ambient')"
        :vr-shading-diffuse="getVrShadingNumber(i-1, 'diffuse')"
        :vr-shading-spec-int="getVrShadingNumber(i-1, 'specularInt')"
        :vr-shading-spec-power="getVrShadingNumber(i-1, 'specularPower')"
    ...
    >
      <div class="mv-sphere-float-hdr" @mousedown="onSphereFloatDragStart" title="ドラッグで移動">
      ...
      <div class="mv-sphere-float-row mv-sphere-radius-row">
        ...
        <span class="mv-sphere-radius-control">
          <v-btn icon="mdi-minus" size="x-small" variant="text" density="compact" title="Decrease radius"
            @click="setSphereRadius(segStore.sphere.radiusMm - 1)" />
          <input class="mv-sphere-radius-input" type="number" min="5" max="200" step="0.5"
            :value="segStore.sphere.radiusMm.toFixed(1)"
            @change="setSphereRadius(($event.target as HTMLInputElement).value)" />
          <span class="mono">mm</span>
          <v-btn icon="mdi-plus" size="x-small" variant="text" density="compact" title="Increase radius"
            @click="setSphereRadius(segStore.sphere.radiusMm + 1)" />
        </span>
      </div>
      <div class="mv-sphere-float-row"><span>ボクセル数</span><span class="mono">{{ segStore.sphere.voxelCount }}</span></div>
    </div>
    ...
    <div v-if="debugMode" class="mv-debug-badge">
      ...
      デバッグ
      <span class="hint">Shift+クリック=ボクセル編集 / Ctrl+Shift+D=切替</span>
    </div>
  </div>
    ...
    <v-card v-if="recoveryCandidate" class="mv-recovery-card">
      ...
      <v-card-text>
        <div class="mv-recovery-line">
          <strong>{{ recoveryCandidate.seriesDescription || '不明なシリーズ' }}</strong>
        </div>
        <div class="mv-recovery-line mv-recovery-meta">
          最終編集 <strong>{{ formatRelativeTime(recoveryCandidate.savedAt) }}</strong>
        ...
        <div class="mv-recovery-line mv-recovery-meta">
          ...
          · {{ recoveryCandidate.labels?.length ?? 0 }} 件のラベル
        </div>
        <div class="mv-recovery-hint">
          保存されたマスクを読み込むと、この PT の現在のセグメンテーションは置き換わります。
        </div>
      </v-card-text>
      <v-card-actions>
        <v-btn variant="text" @click="onRecoverDiscard">
          保存を破棄
          <v-tooltip activator="parent" location="top">自動保存されたマスクを削除して、この通知を表示しないようにします</v-tooltip>
        </v-btn>
        <v-spacer />
        <v-btn variant="text" @click="onRecoverSkip">
          スキップ
          <v-tooltip activator="parent" location="top">保存されたマスクはディスクに残しますが、今回は読み込みません</v-tooltip>
        </v-btn>
        <v-btn variant="flat" color="primary" @click="onRecoverYes">
          復元
          <v-tooltip activator="parent" location="top">自動保存されたマスクを読み込み、現在のセグメンテーションを置き換えます</v-tooltip>
        </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  ...
  <v-dialog v-model="fusionDialog.open" max-width="640">
    <v-card>
      <v-card-title class="text-body-1">
        重ね合わせ先: <span class="text-primary">{{ fusionDialog.baseLabel }}</span>
      </v-card-title>
      <v-card-text class="pt-0">
        <div class="text-caption text-disabled mb-2">
          重ねるシリーズを選択してください。<b>推奨</b> と表示された項目はこのボックスと同じ参照座標系を共有しており、既に空間的に整列しています。
        </div>
        <v-list density="compact" class="mv-fusion-list">
          ...
          >
            <template #prepend>
              ...
              <v-list-item-title>
              {{ c.label }}
              <v-chip v-if="c.recommended" size="x-small" color="primary" variant="flat" class="ml-2">推奨</v-chip>
            </v-list-item-title>
            <v-list-item-subtitle>{{ c.nFrames }} フレーム · {{ c.reason }}</v-list-item-subtitle>
          </v-list-item>
        </v-list>
      </v-card-text>
      <v-card-actions>
        ...
        <v-btn variant="text" @click="fusionDialog.open = false">
          キャンセル
          <v-tooltip activator="parent" location="top">重ね合わせせずに閉じる</v-tooltip>
        </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
    ...
    <v-card>
      <v-card-title class="d-flex align-center" style="gap: 8px;">
        ...
        <v-spacer />
          ...
          <v-icon icon="mdi-close" />
          <v-tooltip activator="parent" location="bottom">NIfTI ヘッダビューアを閉じる</v-tooltip>
        </v-btn>
      </v-card-title>
