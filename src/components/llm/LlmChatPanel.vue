...
<template>
  <div v-if="open" class="mv-llm-panel" @mousedown.stop @wheel.stop @dblclick.stop>
    <div class="mv-llm-head">
      ...
      <span class="mv-llm-title">アシスタント</span>
      <span class="mv-llm-conn" :class="'is-' + connState"
        ...
        <v-tooltip activator="parent" location="bottom">接続設定</v-tooltip>
      </v-btn>
      <v-btn icon variant="text" size="x-small" :disabled="messages.length === 0" @click="clearChat">
        <v-icon icon="mdi-broom" size="x-small" />
        <v-tooltip activator="parent" location="bottom">会話をクリア</v-tooltip>
      </v-btn>
    ...
    <div v-if="showSettings" class="mv-llm-settings">
      ...
      <select v-if="models.length" v-model="settings.model" @change="onSettingsChanged">
        <option v-for="m in models" :key="m.name" :value="m.name">
          {{ m.name }}{{ m.thinking ? '（推論）' : '' }}{{ m.tools ? '' : ' — ツールなし' }}
        </option>
      </select>
      ...
      <select v-if="models.length" v-model="settings.toolModel" @change="onSettingsChanged"
              title="アシスタントがビューアを参照する際に使用するモデル">
        <option value="">（モデルと同じ）</option>
        <option v-for="m in models" :key="'t'+m.name" :value="m.name" :disabled="!m.tools">
          {{ m.name }}{{ m.tools ? '' : ' — ツール非対応' }}
        </option>
      </select>
      <input v-else v-model="settings.toolModel" @change="onSettingsChanged" placeholder="（上と同じ）" />
      <span></span>
      <span class="mv-llm-hint">
        推論モデルはツール結果で応答が遅くなることがあります — 例: qwen3:4b は同質問で 140 秒以上かかりました。ここでは非推論モデルを選ぶことを推奨します。
      </span>
      ...
      <label class="mv-llm-check">
        ...
        <span v-if="supportsThinking(settings.model)">モデルの推論（考え）を別の折りたたみ可能なブロックに保持します。オフにしても qwen3 が速くなるわけではなく、推論を回答に混ぜるだけです。</span>
        <span v-else>{{ settings.model }} は推論モードがないため、影響はありません。</span>
      </label>
      <div v-if="connState === 'ng'" class="mv-llm-err">
        Ollama に接続できません: {{ connReason }}<br />
        <code>ollama serve</code> を実行して起動し、その後モデルをプルしてください（例: <code>ollama pull qwen3:4b</code>）。
      </div>
      <div v-else-if="connState === 'ok' && models.length === 0" class="mv-llm-err">
        接続はできましたが、モデルがインストールされていません。<code>ollama pull qwen3:4b</code> を実行してください。
      </div>
    </div>

    <div class="mv-llm-body" ref="bodyEl">
      <div v-if="messages.length === 0" class="mv-llm-empty">
        このマシン上で動くローカルモデルとチャットできます。<br />
        インターネットには送信されません。
      </div>
      ...
      <div v-if="m.role !== 'tool'" class="mv-llm-msg" :class="'is-' + m.role">
        ...
        <div v-if="m.thinking" class="mv-llm-think">
          <button class="mv-llm-think-toggle" @click="showThinking[i] = !showThinking[i]">
            {{ showThinking[i] ? '▾' : '▸' }}
            {{ busy && i === messages.length - 1 && !m.content ? '考え中…' : '思考' }}
          </button>
        ...
        <div v-else-if="busy && i === messages.length - 1" class="mv-llm-waiting">
          {{ m.toolCalls?.length ? 'ビューアを参照中…' : m.thinking ? '推論中…' : 'モデルの応答を待っています…' }}
        </div>
      </div>
      </template>
    </div>
    ...
    <div class="mv-llm-input">
      ...
      <v-btn v-else icon variant="text" size="small" @click="stop">
        ...
        <v-tooltip activator="parent" location="top">生成を停止</v-tooltip>
      </v-btn>
    </div>
  </div>
</template>
