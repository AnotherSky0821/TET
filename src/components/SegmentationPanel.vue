...
type SegmentationTool = 'window' | 'pan' | 'zoom' | 'page' | 'sphereROI' | 'assignLabel';

const emit = defineEmits<{
    (e: 'redraw'): void;
    (e: 'jump', p: THREE.Vector3): void;
    (e: 'set-tool', tool: SegmentationTool): void;
...
<template>
    <div class="mv-seg-panel">
        <div v-if="!petAvailable" class="mv-empty">
            ...
            PET ボリュームを読み込み、セグメンテーションツールを使うには MPR / Fusion 表示に切り替えてください
        </div>
        ...
        <template v-else>
            ...
            <div v-if="store.petVolumeRef" class="mv-linked-pt">
                ...
                <span class="mv-linked-pt-label">リンクされた PT:</span>
            ...
            <div v-if="suvWarning" class="mv-suv-warning" :title="`source: ${suvWarning.source}`">
                ...
                <div class="mv-suv-warning-text">
                    <strong>SUV 未計算</strong>
                    ボクセル値はそのまま表示されています。
                    <span class="mv-suv-reason">理由: {{ suvWarning.reason }}</span>
                </div>
            </div>
            <div
                v-else-if="suvOkLabel"
                class="mv-suv-ok mv-suv-ok-clickable"
                :title="`source: ${store.petVolumeRef?.metadata?.suvSource} — クリックで SUV 計算の詳細を表示/非表示`"
            ...
            >
                <div class="mv-suv-sanity-head" @click="showSuvDetails = !showSuvDetails">
                    ...
                    <span class="mv-suv-sanity-title">
                        {{ suvSanityHasError
                            ? `${suvSanityWarnings.filter(w => w.severity === 'error').length} 件のエラー`
                            : suvSanityHasWarn
                                ? `${suvSanityWarnings.filter(w => w.severity === 'warn').length} 件の警告`
                                : `${suvSanityWarnings.length} 件の注記` }}
                        （PET メタデータ）
                    </span>
                    <v-icon
            ...
            <div v-if="store.petVolumeRef?.metadata?.modality === 'PT' && showSuvDetails" class="mv-suv-details">
                ...
                <div v-if="suvMetaSummary.suvMode" class="mv-suv-mode-row">
                    ...
                    >
                        <v-btn value="voxbase" size="x-small" title="減衰時間を整数秒に切り捨てます（Vox-BASE と一致）。既定">Vox-BASE</v-btn>
                        <v-btn value="precise" size="x-small" title="小数秒を使用します（物理的により正確）。">Precise</v-btn>
                    </v-btn-toggle>
                </div>
            ...
            <div class="mv-label-list">
                ...
                <div v-if="!showInlineAddLabel" class="mv-label-add-row">
                    <button
                        ...
                        title="新しいラベルを追加（例: リンパ、骨、肺、肝臓）"
                        @click="onOpenInlineAdd"
                    >
                        <v-icon icon="mdi-plus" size="x-small" class="mr-1" />ラベルを追加
                    </button>
                </div>
                <div v-else class="mv-label-add-row is-editing">
                    ...
                    <v-btn size="x-small" variant="tonal" @click="onAddLabelInline">
                        ...
                        <v-tooltip activator="parent" location="bottom">このラベルを追加</v-tooltip>
                    </v-btn>
            ...
            <section class="mv-section">
                <div class="mv-section-title mv-history-head" @click="showHistoryList = !showHistoryList">
                    ...
                    <div class="mv-history-actions" @click.stop>
                        <v-btn size="x-small" variant="text" icon :disabled="!store.canUndo" @click="onUndo" title="元に戻す（Ctrl+Z）">
                            <v-icon icon="mdi-undo" size="small" />
                        </v-btn>
                        <v-btn size="x-small" variant="text" icon :disabled="!store.canRedo" @click="onRedo" title="やり直す（Ctrl+Shift+Z）">
                ...
                <template v-if="showHistoryList">
                    <div v-if="historyTimeline.length === 0" class="mv-hint">
                        まだ編集はありません。しきい値適用、塗りつぶし、ラベル付与で履歴を作成できます。
                    </div>
                    <div v-else class="mv-history-list">
                        <div
                            ...
                            :title="h.applied ? 'ここまで元に戻すにはクリック' : 'ここまでやり直すにはクリック'"
                        >
                            <v-icon
            ...
            <section class="mv-section">
                ...
                <div v-if="needsLiverReference || showRefSpheres" class="mv-ref-spheres mt-2">
                    <div class="mv-ref-row">
                        <v-icon
                            ...
                            size="x-small"
                        />
                        <span class="mv-ref-label">肝臓</span>
                        ...
                        <v-btn
                            ...
                            class="ml-auto"
                        >
                            {{ store.referencePlacementMode === 'liver' ? '肝臓をクリック…' : '配置' }}
                            <v-tooltip activator="parent" location="bottom" max-width="260">
                                {{ store.referencePlacementMode === 'liver'
                                    ? '任意の Volume ボックス上で肝臓内をクリックして 3 cm 参照球を配置してください（再度ここをクリックでキャンセル）'
                                    : '肝臓参照球を配置します — PERCIST / Deauville の閾値に使用されます' }}
                            </v-tooltip>
                        </v-btn>
                    </div>
                    <div class="mv-ref-row mt-1">
                        <v-icon
                            ...
                            size="x-small"
                        />
                        <span class="mv-ref-label">血池（大動脈）</span>
                        ...
                        <v-btn
                            ...
                            class="ml-auto"
                        >
                            {{ store.referencePlacementMode === 'bloodPool' ? '大動脈をクリック…' : '配置' }}
                            <v-tooltip activator="parent" location="bottom" max-width="260">
                                {{ store.referencePlacementMode === 'bloodPool'
                                    ? '任意の Volume ボックス上で大動脈内をクリックして血池参照球を配置してください（再度ここをクリックでキャンセル）'
                                    : '血池（大動脈）参照球を配置します — Deauville スコアリングで使用されます' }}
                            </v-tooltip>
                        </v-btn>
                    </div>
                        <div v-if="store.referencePlacementMode" class="mv-hint mt-1" style="color: var(--mv-warning, #FFB454)">
                        Sphere VOI ツールを使い、{{ store.referencePlacementMode === 'liver' ? '右肝葉' : '下行大動脈' }} をクリックしてください（任意の Volume ボックス）。
                    </div>
                </div>
                ...
                <div class="mv-apply-row mt-2" data-demo="apply-threshold">
                    ...
                    >
                        <v-icon icon="mdi-play" size="small" class="mr-1" />全身をセグメント
                        <v-tooltip activator="parent" location="bottom" max-width="280">
                            PET 全体で閾値を超えるすべてのボクセルを {{ currentLabelName }} としてラベル付けします。手動編集は保持されます。
                        </v-tooltip>
                    </v-btn>
                    <v-btn size="small" variant="outlined" @click="onClearThreshold">
                        クリア
                        <v-tooltip activator="parent" location="bottom" max-width="280">
                            全身閾値マスクをクリアします（確認を求めます）。手動編集は保持されます。
                        </v-tooltip>
                    </v-btn>
                </div>
            </section>
            ...
            <div class="mv-step-head"><span class="mv-step-num">2</span>手動編集</div>
            <section class="mv-section">
                ...
                <div class="mv-tool-toggle-wrap">
                    ...
                    >
                        ...
                        <v-btn value="assignLabel" size="small">
                            <v-icon icon="mdi-tag-outline" size="small" class="mr-1" />割当
                            <v-tooltip activator="parent" location="bottom" open-delay="250" max-width="260">
                                割当: 任意の Volume ボックス上で病変をクリックすると、その 3D 島全体を {{ currentLabelName }} に再ラベルします。
                            </v-tooltip>
                        </v-btn>
                    </v-btn-toggle>
                </div>
            ...
            <section class="mv-section mv-save-row">
                ...
                >
                    <v-icon icon="mdi-content-save" size="small" class="mr-1" />NIfTIマスクを保存
                    <v-tooltip activator="parent" location="top" max-width="280">
                        マルチラベルマスクを .nii + .json サイドカーとしてダウンロード（PET 格子、PET と同じアフィン）
                    </v-tooltip>
                </v-btn>
                <v-menu location="top">
                    ...
                    <v-list density="compact">
                        <v-list-item @click="emit('save-snapshot')">
                            ...
                            <v-list-item-title>セッション全体（.json）</v-list-item-title>
                            <v-list-item-subtitle>レイアウト + マスク + ラベル + ROI</v-list-item-subtitle>
                        </v-list-item>
                        <v-list-item :disabled="!lesionTotals" @click="onExportLesionCsv">
                            ...
                            <v-list-item-title>病変 CSV</v-list-item-title>
                        </v-list-item>
                        <v-list-item :disabled="!store.hasPet || pdfBusy" @click="onExportPdf">
                            ...
                            <v-list-item-title>{{ pdfBusy ? '…' : 'PDF レポート' }}</v-list-item-title>
                            <v-list-item-subtitle>病変テーブル（全行） + 画像</v-list-item-subtitle>
                        </v-list-item>
                        <v-list-item :disabled="!store.hasPet || pptBusy" @click="onExportPpt">
                            ...
                            <v-list-item-title>{{ pptBusy ? '…' : 'PowerPoint スライド' }}</v-list-item-title>
                            <v-list-item-subtitle>画像 + 全身サマリーのみ</v-list-item-subtitle>
                        </v-list-item>
                    </v-list>
                </v-menu>
            </section>
