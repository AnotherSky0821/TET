  ...
  <Teleport to="body">
    <div v-if="d.active.value" class="mv-demo-root" aria-live="polite">
      ...
      <div class="mv-demo-controls">
        <button class="mv-demo-btn" title="最初から再生" @click="d.restart()">
          <v-icon icon="mdi-restart" size="20" />
        </button>
        <button class="mv-demo-btn" title="前へ" :disabled="d.index.value === 0" @click="d.prev()">
          <v-icon icon="mdi-skip-previous" size="20" />
        </button>
        <button v-if="d.playing.value" class="mv-demo-btn primary" title="一時停止" @click="d.pause()">
          <v-icon icon="mdi-pause" size="22" />
        </button>
        <button v-else class="mv-demo-btn primary" title="再生" @click="d.play()">
          <v-icon icon="mdi-play" size="22" />
        </button>
        <button class="mv-demo-btn" title="次へ" :disabled="d.atEnd.value" @click="d.next()">
          <v-icon icon="mdi-skip-next" size="20" />
        </button>
        <div class="mv-demo-sep" />
        <button class="mv-demo-btn exit" title="ツアーを終了（Esc）" @click="d.stop()">
