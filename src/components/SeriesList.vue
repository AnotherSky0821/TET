...
const onCardDragStart = (e: DragEvent, index: number) => {
    ...
    // Chrome/Edge が独自 MIME のみの drag を no-drop 扱いしないよう標準形式も併記する。
    e.dataTransfer.setData('text/plain', `metavol-series:${index}`);
    e.dataTransfer.effectAllowed = 'copy';
    document.body.classList.add('mv-dragging-series');
};

const onCardDragEnd = () => {
    document.body.classList.remove('mv-dragging-series');
};
...
<template>
    <v-container fluid class="pa-1">
        ...
        >
            <v-tooltip activator="parent" location="right">このカードを画像ボックスへドラッグしてシリーズを表示</v-tooltip>
            <!-- ... メニュー: NIfTI raw bytes 表示 / NIfTI header 表示。
                 sourceType=NIFTI のときだけ NIfTI 系項目を表示。 -->
            <v-menu location="bottom end" :title="'このカードを画像ボックスにドラッグしてシリーズを読み込みます'">
                ...
                <v-list density="compact">
                    ...
                    >
                        ...
                        <v-list-item-title>NIfTI ヘッダを表示</v-list-item-title>
                    </v-list-item>
                    <v-list-item
                    ...
                    >
                        ...
                        <v-list-item-title>生バイトを検査</v-list-item-title>
                    ...
                    <v-list-item @click="emit('exportNifti', { index: s.index, gzip: true })">
                        ...
                        <v-list-item-title>NIfTI としてエクスポート（.nii.gz）</v-list-item-title>
                        <v-list-item-subtitle>画像とサイドカーを含む .zip をダウンロードします</v-list-item-subtitle>
                    </v-list-item>
                    <v-list-item @click="emit('exportNifti', { index: s.index, gzip: false })">
                        <template v-slot:prepend>
                            <v-icon icon="mdi-download" size="small" />
                        </template>
                        <v-list-item-title>NIfTI としてエクスポート（.nii）</v-list-item-title>
                        <v-list-item-subtitle>.zip 内に非圧縮の画像を含みます</v-list-item-subtitle>
                    </v-list-item>
                </v-list>
            </v-menu>
            <div
