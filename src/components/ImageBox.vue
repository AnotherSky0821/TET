...
const prop = defineProps<{
  ...
  windowValues?: { wc: number; ww: number; unit: string } | null;
  windowValues2?: { wc: number; ww: number; unit: string } | null;
...
const emit = defineEmits<{
  ...
  // Sidebar の series card / OS ファイルの drop を親 (DicomView) へ渡す
  (e: 'drop', ev: DragEvent): void;
  (e: 'boxDragStart', ev: DragEvent): void;
  (e: 'moveBox', direction: 'left' | 'right' | 'up' | 'down'): void;
  (e: 'resizeBox', payload: { axis: 'row' | 'column'; delta: number }): void;
  (e: 'toggleResizeMode'): void;
  (e: 'resizeDragStart', ev: PointerEvent): void;
  (e: 'setWindow', payload: { layer: 'base' | 'overlay'; wc: number; ww: number }): void;
...
const onDragOver = (e: DragEvent) => {
  e.preventDefault();
  if (e.dataTransfer) {
    e.dataTransfer.dropEffect = e.dataTransfer.types.includes('application/x-metavol-box')
      ? 'move'
      : 'copy';
  }
};
...
const windowEditorOpenBase = ref(false);
const windowEditorOpenOverlay = ref(false);
const windowEditorLayer = ref<'base' | 'overlay'>('base');
const editWc = ref(0);
const editWw = ref(1);

const setWindowEditorOpen = (layer: 'base' | 'overlay', open: boolean) => {
  if (layer === 'base') {
    windowEditorOpenBase.value = open;
    if (open) windowEditorOpenOverlay.value = false;
  } else {
    windowEditorOpenOverlay.value = open;
    if (open) windowEditorOpenBase.value = false;
  }
};

const openWindowEditor = (layer: 'base' | 'overlay') => {
  const values = layer === 'overlay' ? prop.windowValues2 : prop.windowValues;
  if (!values) return;
  editWc.value = values.wc;
  editWw.value = values.ww;
  windowEditorLayer.value = layer;
  setWindowEditorOpen(layer, true);
};

const applyWindowEditor = () => {
  const isOpen = windowEditorLayer.value === 'base' ? windowEditorOpenBase.value : windowEditorOpenOverlay.value;
  if (!isOpen || !Number.isFinite(editWc.value) || !Number.isFinite(editWw.value)) return;
  emit('setWindow', { layer: windowEditorLayer.value, wc: editWc.value, ww: Math.max(0.000001, editWw.value) });
  setWindowEditorOpen(windowEditorLayer.value, false);
};

const applyWindowInput = (layer: 'base' | 'overlay', field: 'wc' | 'ww', raw: number | string | null | undefined) => {
  const values = layer === 'overlay' ? prop.windowValues2 : prop.windowValues;
  if (!values) return;
  const value = Number(raw ?? (field === 'wc' ? values.wc : values.ww));
  if (!Number.isFinite(value)) return;
  const next = {
    wc: field === 'wc' ? value : values.wc,
    ww: field === 'ww' ? Math.max(0.000001, value) : values.ww,
  };
  emit('setWindow', { layer, wc: next.wc, ww: next.ww });
};

const applyWindowRangeInput = (layer: 'base' | 'overlay', bound: 'low' | 'high', raw: number | string | null | undefined) => {
  const values = layer === 'overlay' ? prop.windowValues2 : prop.windowValues;
  if (!values) return;
  const low = values.wc - values.ww / 2;
  const high = values.wc + values.ww / 2;
  const value = Number(raw ?? (bound === 'low' ? low : high));
  if (!Number.isFinite(value)) return;
  const nextLow = bound === 'low' ? value : low;
  const nextHigh = bound === 'high' ? value : high;
  const nextWc = (nextLow + nextHigh) / 2;
  const nextWw = Math.max(0.000001, nextHigh - nextLow);
  emit('setWindow', { layer, wc: nextWc, ww: nextWw });
};

const planeItems = [
  { id: 'axi',  label: '横断 (Axial)' },
  { id: 'cor',  label: '冠状 (Coronal)' },
  { id: 'sag',  label: '矢状 (Sagittal)' },
  { id: 'mip',  label: 'MIP' },
  { id: 'smip', label: 'sMIP' },
  { id: 'vr',   label: 'ボリュームレンダリング' },
] as const;
...
const show = (ppp: Float32Array | Int16Array, cols: number, rows: number, wc: number, ww: number, intercept: number, slope: number, centerX:number, centerY:number, zoom: number, interpolation: 'nearest' | 'bilinear' = 'bilinear', clut?: number[][]) => {
    isEmpty.value = false;
    drawImageCvZoom(ppp, cols, rows, wc, ww, intercept, slope, centerX, centerY, zoom, interpolation, clut);
}
...
const drawImageCvZoom = async function(pix: Float32Array | Int16Array, ny:number, nx:number, wc:number, ww:number, intercept:number, slope:number, shiftX:number, shiftY:number, zoom:number, interpolation: 'nearest' | 'bilinear' = 'bilinear', clut?: number[][]) {
  ...
  for (let y = 0; y<canvasy; y++){
    ...
    for (let x = 0; x<canvasx; x++){
      ...
      if (p>255) p=255;
      if (useClut) {
        const rgb = clut![p] ?? [p, p, p];
        myImageData.data[ad] = rgb[0];
        myImageData.data[ad+1] = rgb[1];
        myImageData.data[ad+2] = rgb[2];
      } else {
...
<template>
    <div class="drop_area mv-box"
      @dragover="onDragOver"
      @drop.prevent.stop="(e: DragEvent) => emit('drop', e)"
        ...
        <div class="mv-titlebar"
          ...
          <span
            class="mv-box-drag-handle"
            draggable="true"
            @dragstart="(e: DragEvent) => emit('boxDragStart', e)"
            @click.stop
          >
            <v-icon icon="mdi-drag-vertical" size="small" />
            <v-tooltip activator="parent" location="bottom">ドラッグしてボックスの順番を変更</v-tooltip>
          </span>
            <span class="mv-mod-chip"
                  :class="{ 'mv-mod-chip--draggable': isModalityChipDraggable() }"
                  :draggable="isModalityChipDraggable()"
                  :title="isModalityChipDraggable() ? '別のボックスにドラッグして融合' : ''"
            ...
            <span class="mv-titlebar-actions" @dblclick.stop>
              <v-menu location="bottom end">
                <template v-slot:activator="{ props: menuProps }">
                  <v-btn v-bind="menuProps" icon variant="text" size="x-small" class="mv-tb-btn">
                    <v-icon icon="mdi-arrange-bring-forward" size="small" />
                    <v-tooltip activator="parent" location="bottom">ボックスのレイアウトを変更</v-tooltip>
                  </v-btn>
                </template>
                <v-list density="compact">
                  <v-list-subheader>ボックスを移動</v-list-subheader>
                  <v-list-item @click="emit('moveBox', 'left')"><v-list-item-title>左へ移動</v-list-item-title></v-list-item>
                  <v-list-item @click="emit('moveBox', 'right')"><v-list-item-title>右へ移動</v-list-item-title></v-list-item>
                  <v-list-item @click="emit('moveBox', 'up')"><v-list-item-title>上へ移動</v-list-item-title></v-list-item>
                  <v-list-item @click="emit('moveBox', 'down')"><v-list-item-title>下へ移動</v-list-item-title></v-list-item>
                  <v-divider />
                  <v-list-subheader>ボックスのサイズ</v-list-subheader>
                  <v-list-item @click="emit('toggleResizeMode')"><v-list-item-title>{{ prop.resizeMode ? 'マウスでのサイズ変更を終了' : 'マウスでサイズ変更' }}</v-list-item-title></v-list-item>
                  <v-list-item @click="emit('resizeBox', { axis: 'column', delta: 1 })"><v-list-item-title>横幅を広げる</v-list-item-title></v-list-item>
                  <v-list-item @click="emit('resizeBox', { axis: 'column', delta: -1 })"><v-list-item-title>横幅を狭める</v-list-item-title></v-list-item>
                  <v-list-item @click="emit('resizeBox', { axis: 'row', delta: 1 })"><v-list-item-title>高さを増やす</v-list-item-title></v-list-item>
                  <v-list-item @click="emit('resizeBox', { axis: 'row', delta: -1 })"><v-list-item-title>高さを減らす</v-list-item-title></v-list-item>
                </v-list>
              </v-menu>
              ...
              <Teleport :to="overflowEl" :disabled="!isCompact || !overflowEl">
                ...
                <v-menu v-if="isReconstructed()" location="bottom end">
                    <template v-slot:activator="{ props: act }">
                        <v-btn v-bind="act" icon variant="text" size="x-small" class="mv-tb-btn">
                            ...
                            <v-tooltip activator="parent" location="bottom">断面 / 表示</v-tooltip>
                        </v-btn>
                    </template>
                    <v-list density="compact">
                        ...
                        <template v-if="prop.canRevertToDicom">
                            ...
                            <v-list-item @click="emit('backToDicom')">
                                ...
                                <v-list-item-title>オリジナル画像</v-list-item-title>
                                <v-list-item-subtitle>原画に戻す</v-list-item-subtitle>
                            </v-list-item>
                        </template>
                    </v-list>
                </v-menu>
                ...
                <v-menu v-else-if="prop.rendering === 'native'" location="bottom end">
                    <template v-slot:activator="{ props: act }">
                        <v-btn v-bind="act" icon variant="text" size="x-small" class="mv-tb-btn">
                            <v-icon icon="mdi-axis-arrow" size="small" />
                            <v-tooltip activator="parent" location="bottom">MPR を作成（このボックス）</v-tooltip>
                        </v-btn>
                    </template>
                    <v-list density="compact">
                        <v-list-item v-for="p in planeItems" :key="p.id"
                ...
                <!-- CLUT (single layer: native DICOM / Volume / MIP / VR) -->
                <v-menu v-if="(!prop.fused) && (prop.rendering === 'native' || isReconstructed())" location="bottom end">
                    <template v-slot:activator="{ props: act }">
                        <v-btn v-bind="act" icon variant="text" size="x-small" class="mv-tb-btn">
                            <v-icon icon="mdi-palette" size="small" />
                            <v-tooltip activator="parent" location="bottom">カラー（CLUT）</v-tooltip>
                        </v-btn>
                    </template>
                    <v-list density="compact">
                ...
                <v-menu v-if="prop.fused" location="bottom end">
                    <template v-slot:activator="{ props: act }">
                        <v-btn v-bind="act" icon variant="text" size="x-small" class="mv-tb-btn mv-tb-clut-base">
                            ...
                            <v-tooltip activator="parent" location="bottom">カラー（CLUT）— ベース（{{ prop.baseModality || '' }})</v-tooltip>
                        </v-btn>
                    </template>
                    <v-list density="compact">
                ...
                <v-menu v-if="prop.fused" location="bottom end">
                    <template v-slot:activator="{ props: act }">
                        <v-btn v-bind="act" icon variant="text" size="x-small" class="mv-tb-btn mv-tb-clut-ovl">
                            ...
                            <v-tooltip activator="parent" location="bottom">カラー（CLUT）— オーバーレイ（{{ prop.overlayModality || '' }})</v-tooltip>
                        </v-btn>
                    </template>
                    <v-list density="compact">
                ...
                >
                    <template v-slot:activator="{ props: act }">
                        <v-btn v-bind="act" icon variant="text" size="x-small" class="mv-tb-btn">
                            ...
                            <v-tooltip activator="parent" location="bottom">サンプリング（補間）</v-tooltip>
                        </v-btn>
                    </template>
                        ...
                        <v-menu open-on-hover location="end">
                            <template v-slot:activator="{ props: a }">
                                <v-list-item v-bind="a">
                                    ...
                                    <v-list-item-title>ベース ({{ (prop.interpolation ?? 'bilinear') }})</v-list-item-title>
                            ...
                            <v-list density="compact">
                                ...
                                <v-list-item :active="prop.interpolation === 'nearest'"
                                             @click="emit('setInterpolation', { layer: 'base', mode: 'nearest' })">
                                    <v-list-item-title>ニアレスト（最近傍）</v-list-item-title>
                                </v-list-item>
                            </v-list>
                        </v-menu>
                        <v-menu open-on-hover location="end">
                            <template v-slot:activator="{ props: a }">
                                <v-list-item v-bind="a">
                                    ...
                                    <v-list-item-title>オーバーレイ ({{ (prop.interpolation1 ?? 'bilinear') }})</v-list-item-title>
                                    <template v-slot:append>
                                        <v-icon icon="mdi-chevron-right" size="small" />
                                    </template>
                                </v-list-item>
                ...
                <!-- VR demo 再生ボタン (VR plane のみ表示)。
                    ...
                    <v-tooltip activator="parent" location="bottom">
                      {{ prop.vrDemoRunning ? 'デモを停止' : 'VR デモを再生（約30秒）' }}
                    </v-tooltip>
                </v-btn>
                ...
                <v-menu v-if="prop.currentPlane === 'smip' || prop.currentPlane === 'vr'"
                    ...
                    <template v-slot:activator="{ props: act }">
                        <v-btn v-bind="act" icon variant="text" size="x-small" class="mv-tb-btn">
                            ...
                            <v-tooltip activator="parent" location="bottom">
                              {{ prop.currentPlane === 'smip' ? 'Surface MIP パラメータ' : 'VR パラメータ' }}
                            </v-tooltip>
                        </v-btn>
                    </template>
                        ...
                        <v-card-text class="pa-3">
                            ...
                            <template v-if="prop.currentPlane === 'smip'">
                                <div class="mv-tb-popover-label">
                                    Surface threshold
                                    <button class="mv-tb-auto" title="このボリュームのヒストグラムから再計算"
                                ...
                                <div class="mv-tb-slider-row">
                                    ...
                                    <div class="mv-tb-popover-label mt-3">シェーディング</div>
                            ...
                            <template v-else>
                                ...
                                <div class="text-caption text-disabled mt-2 mb-3">
                                  プリセットは値→不透明度カーブを決定します。Alpha scale は全体スケールです。
                                </div>
                                ...
                                <label class="mv-tb-shading-toggle">
                                    ...
                                    />
                                    <span>シェーディング（Phong）</span>
                                    <span class="text-caption text-disabled ml-1">— 高負荷（GPU 必須）</span>
                                </label>
                ...
                >
                    ...
                    <v-tooltip activator="parent" location="bottom">
                      W/L ドラッグ対象: {{ (prop.activeWindowLayer === 'base') ? prop.baseModality : prop.overlayModality }}（クリックで切替）
                    </v-tooltip>
                </v-btn>
                ...
                <div v-if="prop.fused" class="mv-tb-blend">
                    ...
                    />
                    <v-tooltip activator="parent" location="bottom">
                      オーバーレイ {{ Math.round((prop.overlayAlpha ?? 0.5) * 100) }}% / ベース {{ 100 - Math.round((prop.overlayAlpha ?? 0.5) * 100) }}%
                    </v-tooltip>
                </div>
                ...
                <v-btn icon variant="text" size="x-small" class="mv-tb-btn"
                    ...
                    <v-tooltip activator="parent" location="bottom">WL/表示をリセット</v-tooltip>
                </v-btn>
                ...
                <v-btn v-if="prop.globalSyncOn"
                    ...
                    <v-tooltip activator="parent" location="bottom">
                      {{ prop.syncEnabled ? 'このボックスは同期中（クリックで解除）' : 'このボックスは同期オフ（クリックで接続）' }}
                    </v-tooltip>
                </v-btn>
                ...
                <v-btn icon variant="text" size="x-small" class="mv-tb-btn"
                    ...
                    <v-tooltip activator="parent" location="bottom">最大化 / 復元</v-tooltip>
                </v-btn>
              </Teleport>
                ...
                <v-btn v-if="isCompact" icon variant="text" size="x-small"
                    ...
                    <v-tooltip activator="parent" location="bottom">ボックス操作</v-tooltip>
                </v-btn>
                ...
                <v-menu location="bottom end">
                    <template v-slot:activator="{ props: act }">
                        <v-btn v-bind="act" icon variant="text" size="x-small" class="mv-tb-btn">
                            ...
                            <v-tooltip activator="parent" location="bottom">その他</v-tooltip>
                        </v-btn>
                    </template>
                    <v-list density="compact">
                        <v-list-item @click="emit('duplicateBox')">
                          <v-list-item-title>このボックスを複製</v-list-item-title>
                          <v-list-item-subtitle>同じ表示で新しいボックスを作成</v-list-item-subtitle>
                        </v-list-item>
                        <v-divider />
                        <v-list-item @click="onSavePngLocal">
                          <v-list-item-title>PNG を保存</v-list-item-title>
                          <v-list-item-subtitle>この表示のスクリーンショット</v-list-item-subtitle>
                        </v-list-item>
                        <v-list-item @click="emit('saveVolumeNifti')">
                          <v-list-item-title>ボリュームを NIfTI として保存</v-list-item-title>
                          <v-list-item-subtitle>PT=SUV / CT=HU と JSON サイドカー</v-list-item-subtitle>
                        </v-list-item>
                        <v-list-item @click="emit('toggleOverlay')">
                          <v-list-item-title>マスクオーバーレイ切替</v-list-item-title>
                        </v-list-item>
                        <v-divider />
                        <v-list-item @click="emit('pickFusion')">
                            <template #prepend><v-icon icon="mdi-layers-plus" size="small" /></template>
                          <v-list-item-title>融合するシリーズを選択…</v-list-item-title>
                          <v-list-item-subtitle>このボックスに重ねるシリーズを選択</v-list-item-subtitle>
                        </v-list-item>
                        ...
                        <template v-if="prop.fused">
                            ...
                            <v-list-item :disabled="prop.registerBusy" @click="emit('autoRegister')">
                                ...
                                <v-list-item-title>
                                  {{ prop.registerBusy ? '登録中…' : 'オーバーレイ → ベース を自動位置合わせ' }}
                                </v-list-item-title>
                                <v-list-item-subtitle>重ねたシリーズをこのボックスのベースに合わせます（剛体, MI）</v-list-item-subtitle>
                            </v-list-item>
                            ...
                            <v-list-item v-if="prop.manualAlign" :disabled="prop.registerBusy"
                                ...
                                <v-list-item-title>
                                    {{ prop.manualAlign?.active ? '手動位置合わせを完了' : '手動で位置合わせを調整' }}
                                </v-list-item-title>
                            ...
                            <v-list-item :disabled="prop.registerBusy" @click="emit('resetRegister')">
                                <template #prepend><v-icon icon="mdi-restore" size="small" /></template>
                                    <v-list-item-title>位置合わせをリセット</v-list-item-title>
                                <v-list-item-subtitle>オーバーレイを撮影時の位置に戻す</v-list-item-subtitle>
                            </v-list-item>
                            <v-divider />
                ...
                <v-btn icon variant="text" size="x-small" class="mv-tb-btn mv-tb-close"
                    ...
                    <v-tooltip activator="parent" location="bottom">このボックスを閉じる</v-tooltip>
                </v-btn>
            </span>
            <div
              v-if="prop.resizeMode"
              class="mv-resize-handle"
              @pointerdown.stop.prevent="(e: PointerEvent) => emit('resizeDragStart', e)"
            >
              <v-icon icon="mdi-arrow-bottom-right" size="small" />
              <v-tooltip activator="parent" location="left">ドラッグしてサイズを変更</v-tooltip>
            </div>
        </div>
        ...
        <div class="mv-canvas-wrap">
            ...
            <div v-if="prop.manualAlign?.active" class="mv-align-panel"
                ...
                <div class="mv-align-pad">
                    <button class="mv-align-key mv-align-key--rot" title="この断面を反時計回りに回転"
                            @click="emit('manualNudge', { rot: -1 })">⟲</button>
                    <button class="mv-align-key" title="オーバーレイを上へ移動（↑）"
                            @click="emit('manualNudge', { up: 1 })">▲</button>
                    <button class="mv-align-key mv-align-key--rot" title="この断面を時計回りに回転"
                            @click="emit('manualNudge', { rot: 1 })">⟳</button>
                    <button class="mv-align-key" title="オーバーレイを左へ移動（←）"
                            @click="emit('manualNudge', { right: -1 })">◀</button>
                    <span class="mv-align-center mv-mono">{{ prop.manualAlign.stepMm }}</span>
                    <button class="mv-align-key" title="オーバーレイを右へ移動（→）"
                            @click="emit('manualNudge', { right: 1 })">▶</button>
                    <button class="mv-align-key" title="オーバーレイを手前へ移動（PageUp）"
                            @click="emit('manualNudge', { through: -1 })">−z</button>
                    <button class="mv-align-key" title="オーバーレイを下へ移動（↓）"
                            @click="emit('manualNudge', { up: -1 })">▼</button>
                    <button class="mv-align-key" title="オーバーレイを奥へ移動（PageDown）"
                            @click="emit('manualNudge', { through: 1 })">+z</button>
                </div>
                ...
                <div class="mv-align-actions">
                    <button class="mv-align-act mv-align-act--undo"
                            ...
                            title="最後の位置合わせ操作を元に戻す（Ctrl+Z）"
                            @click="emit('undoRegister')">↶</button>
                    <button class="mv-align-act" :disabled="prop.registerBusy"
                            title="ここで設定した姿勢から MI 位置合わせを実行"
                            @click="emit('refineRegister')">
                        {{ prop.registerBusy ? 'Refining…' : 'Refine with auto' }}
                    </button>
                    <button class="mv-align-act" :disabled="prop.registerBusy"
                            title="オーバーレイを撮影時の位置に戻す"
                            @click="emit('resetRegister')">リセット</button>
                </div>
                <div class="mv-align-hint">Arrow keys move · Shift + drag · Ctrl+Z undo</div>
            </div>

            <!-- Scale legend: editable min/max values sit directly at the bar ends -->
            <div v-if="prop.legend" class="mv-clut-legend" :class="{ 'mv-clut-legend--editable': !!prop.windowValues }">
              <template v-if="prop.windowValues">
                <v-text-field
                  :model-value="prop.windowValues.wc - prop.windowValues.ww / 2"
                  type="number"
                  density="compact"
                  hide-details
                  class="mv-window-range-field"
                  @update:model-value="(v: number | string | null | undefined) => applyWindowRangeInput('base', 'low', v)"
                />
              </template>
              <template v-else>
              ...
              <template v-if="prop.windowValues">
                <v-text-field
                  ...
                  hide-details
                  class="mv-window-range-field"
                  @update:model-value="(v: number | string | null | undefined) => applyWindowRangeInput('base', 'high', v)"
                />
              </template>
              <template v-else>
            ...
            <div v-if="prop.legend2" class="mv-clut-legend mv-clut-legend--second" :class="{ 'mv-clut-legend--editable': !!prop.windowValues2 }">
              <template v-if="prop.windowValues2">
                <v-text-field
                  :model-value="prop.windowValues2.wc - prop.windowValues2.ww / 2"
                  type="number"
                  density="compact"
                  hide-details
                  class="mv-window-range-field"
                  @update:model-value="(v: number | string | null | undefined) => applyWindowRangeInput('overlay', 'low', v)"
                />
              </template>
              <template v-else>
              ...
              <template v-if="prop.windowValues2">
                <v-text-field
                  ...
                  hide-details
                  class="mv-window-range-field"
                  @update:model-value="(v: number | string | null | undefined) => applyWindowRangeInput('overlay', 'high', v)"
                />
              </template>
              <template v-else>
...
.mv-titlebar {
  ...
  overflow: hidden;
  background: #edf1f4;
  border-bottom: 1px solid #7892a7;
  font-size: 10px;
...
.mv-box-drag-handle {
  display: inline-flex;
  align-items: center;
  color: var(--mv-text-muted, #8FA0B0);
  cursor: grab;
  flex-shrink: 0;
}
.mv-box-drag-handle:active {
  cursor: grabbing;
}
.mv-box-size-btn {
  width: 18px;
...
.mv-box-size-btn:hover {
  color: var(--mv-accent, #00D4AA);
}
.mv-box.is-selected .mv-titlebar {
  background: linear-gradient(180deg, rgba(22,114,173,0.12) 0%, #edf1f4 100%);
  border-bottom-color: var(--mv-accent, #1672ad);
}
.mv-resize-handle {
  position: absolute;
  right: 3px;
...
.mv-window-editor-title {
  margin-bottom: 6px;
  color: var(--mv-text-muted, #8FA0B0);
  font-size: 11px;
  font-weight: 700;
}
.mv-window-range-field {
  width: 100px;
  flex: 0 0 auto;
  pointer-events: auto;
}
.mv-window-range-field .v-input__control {
  min-height: 22px;
}
.mv-window-range-field .v-field__input {
  min-height: 22px;
  font-size: 5px;
  font-family: 'JetBrains Mono', 'Consolas', monospace;
}
.mv-window-range-field .v-field__overlay {
  min-height: 24px;
}
